...
//  Instagram
//
//  Created by mayuka on 2018/06/10.
...
class HomeViewController: UIViewController,UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.commentButton.addTarget(self, action: #selector(handleCMButton(_:forEvent:)), for: .touchUpInside)
        
        return cell
    }
    
    //セル内のコメントボタンがタップされた時に呼ばれるメソッド
    @objc func handleCMButton(_ sender: UIButton, forEvent event: UIEvent) {
        ...
        if let commentVC = CommentViewController as? CommentViewController {
            //インデックスのデータ(postdata)をコメント入力画面の変数に渡す
